...
```{r, fig.width = 8, fig.height = 6, echo = FALSE, warning = FALSE, message = FALSE, comment = NA, error= FALSE, cache = FALSE}
...
dat_c$outlier <- ifelse(dat_c$text_label == 'SC', 'In Cluster', 
                        ifelse(dat_c$text_label == 'FC', 'In Cluster', dat_c$outlier))
...
names(data) <- c('x', 'y', 'circuit','decade', 'decades')
  ...
  g1 <- ggplot(data,
    ...
    ylim(c(-7, 18)) +
    geom_point(size = 2) +
    geom_dl(data = subset(data, decades ==  1890 | decades == 1950 | decades == 2010), aes(label = decades), method = 'smart.grid') +
