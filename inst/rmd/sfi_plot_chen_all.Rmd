...
temp <- kmod(data[, c('x', 'y')], k = 14, l = 200, i_max = 300, conv_method = "delta_C",
...
# figure 1 (sampled half the data set (n = 1089))
...
data$cluster <- as.character(dat_c$cluster)
...
# get one text_label per group with ggrepel
...
unique_text_label <- unique(data$text_label)
for(i in 1:length(unique_text_label)){
  this_text <- unique_text_label[i]
  sub_text <- data[data$text_label == this_text,]

    
  # randomly assign year label to one row
  sub_text <- sub_text[order(sub_text$clust_dist.dist_sqr, decreasing = FALSE),]
  top_ten <- 1
  sub_text$label[top_ten] <- as.character(this_text)
...
g1 <- ggplot(dat,
  ...
  geom_point(size = 1,
            alpha = 0.8) +
  geom_label_repel(aes(label = label), size = 7,segment.size = 0, box.padding = 0.5) +
  scale_color_manual(name = '',
                     values = c('black', 'grey')) +
